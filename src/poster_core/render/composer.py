...
def _dramatic_backdrop(
    ...
    try:
        ...
        return None, None
    canvas = scrim(canvas, start=0.12, strength=0.97)
...
def _render_hook(
    ...
    """Cold open: no category chip, no facts — just enough tension to make
    the first swipe happen. Never repeats the headline verbatim."""
    w, h = size
    margin, g = _margin(size), blocks.grid(size)
...
def _render_evidence(
    ...
    if slide.kicker:
        blocks.kicker(canvas, margin, round(h * 0.08), slide.kicker, theme, brand,
...
def _render_conclusion(
    ...
    draw = ImageDraw.Draw(canvas)
    blocks.chip(canvas, margin, margin, slide.kicker or "WHAT HAPPENS NEXT",
                theme, brand, max(16, h // 56))

    body_font, body_lines = fit_text(
        draw, slide.body or "", brand, round(h * 0.046), w - 2 * margin, max_lines=6
    )
